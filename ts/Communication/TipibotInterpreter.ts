...
export class TipibotInterpreter extends Interpreter {
	...
	convertToZeroCenteredCoordinates(point: paper.Point) {
		let tipibotSize = new paper.Size(Settings.tipibot.width, Settings.tipibot.height)
		let zeroCenteredPoint = point.subtract(tipibotSize.multiply(0.5) as any)
		zeroCenteredPoint.y *= -1
		return zeroCenteredPoint
	}

    sendSetHome(point: paper.Point=this.tipibot.getHome()) {
		super.sendSetHome(point)
		this.lastCommandWasMove = false
		point = this.convertToZeroCenteredCoordinates(point)
